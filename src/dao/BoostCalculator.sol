 ...
 */
contract BoostCalculator is AltheaOwnable, SystemStart {
    ...
    event BoostParamsSet(
        uint256 maxBoostMultiplier, uint256 maxBoostablePct, uint256 decayBoostPct, uint256 paramChangeEpoch
    );
    ...
    constructor(
        ...
        uint16 _maxBoostPct,
        uint16 _decayPct
    ) AltheaOwnable(_prismaCore) SystemStart(_prismaCore) {
        locker = _locker;
     ...
     */
    function getClaimableWithBoost(address account, uint256 previousAmount, uint256 totalEpochEmissions)
        external
        view
        returns (uint256 maxBoosted, uint256 boosted)
    {
        ...
        if (paramChangeEpoch != 0 && paramChangeEpoch <= epoch) {
            ...
            (maxBoostable, fullDecay) =
                _getBoostable(totalEpochEmissions, pct, pendingMaxBoostablePct, pendingDecayBoostPct);
        } else {
     ...
     */
    function getBoostedAmount(address account, uint256 amount, uint256 previousAmount, uint256 totalEpochEmissions)
        external
        view
     ...
     *     @return adjustedAmount Amount of PRISMA received after applying boost
     */
    function getBoostedAmountWrite(address account, uint256 amount, uint256 previousAmount, uint256 totalEpochEmissions)
        external
        returns (uint256 adjustedAmount)
    {
        uint256 epoch = getWeek();
        ...
        (uint256 maxBoostable, uint256 fullDecay) =
            _getBoostable(totalEpochEmissions, lockPct, maxBoostablePct, decayBoostPct);

        return _getBoostedAmount(amount, previousAmount, lockPct, maxBoostMultiplier, maxBoostable, fullDecay);
    }

    function _getBoostable(uint256 totalEpochEmissions, uint256 lockPct, uint256 maxBoostPct, uint256 decayPct)
        internal
        pure
